...
bool list_isEmpty(List* list) {
    if (list == NULL) return true;  // Lista no inicializada
    return (list_first(list) == NULL);  // Si el primer elemento es NULL, está vacía
}
...
int list_remove(List* list, int index) {
    if (list == NULL || index < 0 || index >= list->size) return -1; // Índice inválido
    ...
    // Para índices > 0, avanzamos hasta la posición deseada
    for (int i = 0; i < index; i++) {
        prev = current;
        current = current->next;
    }

    // Reenlazamos los nodos
